...
function normalizeTestToken(value: string): string {
  return value.split(".").length === 3 ? value : codexAccessToken(value === "codex-token" ? "account-123" : value);
}
...
test("missing reset summary keeps the current Codex availability check reachable", async (t) => {
  ...
  const choices: Array<string | undefined> = ["Redeem usage limit reset…", "Full reset", undefined, "Close"];
  const mock = createMockPi();
...
test("Codex reset transport retries reuse the same redemption request ID", async (t) => {
  ...
  const choices = ["Redeem usage limit reset…", "Full reset", "Yes, use reset", "Try again", "Close"];
  const titles: string[] = [];
  const mock = createMockPi();
  usageExtension(mock.pi, {
