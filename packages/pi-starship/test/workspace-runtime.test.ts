...
test("workspace commands stream output and terminate at the byte limit", async () => {
  ...
  const result = await execWorkspaceCommand(
    process.execPath,
    ["-e", "process.stdout.write(Buffer.alloc(2 * 1024 * 1024, 120)); setInterval(() => {}, 10_000);"],
...
test("workspace commands honor caller cancellation", async () => {
  ...
  const started = Date.now();
  const resultPromise = execWorkspaceCommand(process.execPath, ["-e", "setInterval(() => {}, 10_000);"], {
    cwd: process.cwd(),
    timeout: 2_000,
  ...
  try {
    ...
    const snapshot = await collectWorkspaceSnapshot({
      ...
      exec: async (command, args, options) => {
        ...
        if (command === "python") return { stdout: outputs.python ?? "", stderr: "", code: 0, killed: false };
        if (command === "go") return { stdout: outputs.go ?? "", stderr: "", code: 0, killed: false };
        if (command === "bun") return { stdout: outputs.bun ?? "", stderr: "", code: 0, killed: false };
...
test("development environment modules use allowlisted activation data and lazy commands", async () => {
  ...
  try {
    ...
    const snapshot = await collectWorkspaceSnapshot({
      ...
      exec: async (command, args) => {
        calls.push(`${command} ${args.join(" ")}`);
        if (command === "mise") return { stdout: "all checks passed", stderr: "", code: 0, killed: false };
        if (command === "direnv")
          return {
...
test("Azure and OpenStack readers select metadata while discarding adjacent secrets", async () => {
  ...
  try {
    ...
    assert.doesNotMatch(JSON.stringify(snapshot), /SENTINEL|PASSWORD|TOKEN|auth_url|secret\.invalid/iu);
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
});
...
test("oversized and malformed metadata files fail empty without leaking source text", async () => {
  ...
  try {
    ...
    mkdirSync(join(root, ".aws"));
    writeFileSync(join(root, ".aws", "config"), "[profile broken\naws_secret_access_key=SENTINEL_SECRET\n");
    const snapshot = await collectWorkspaceSnapshot({
      cwd: root,
