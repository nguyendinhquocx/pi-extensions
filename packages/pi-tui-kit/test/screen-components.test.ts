...
const detailScreen: MenuScreen<ScreenId, ActionId> = {
  kind: "detail",
  title: "Details",
  lines: ["One very long detail line that must remain visible without crossing the terminal width."],
  hint: "back",
};
...
test("standard screens remain bounded and sanitize terminal controls", () => {
  for (const screen of [actionScreen, detailScreen, choiceScreen, settingsScreen, multiSelectScreen]) {
...
test("action screens prioritize actionable rows when terminal height is constrained", () => {
  const screen: MenuScreen<ScreenId, ActionId> = {
    ...actionScreen,
    lines: ["Sync paused: owner unknown.", "Close other Pi sessions then restore; Settings and More return."],
    items: [
...
test("choice screens use injected navigation, block disabled activation, and distinguish exits", () => {
  ...
  assert.match(stripVTControlCharacters(disabled.component.render(80).join("\n")), /Unavailable here/);
...
test("choice screens page through a bounded viewport and sanitize all displayed text", () => {
  const items = Array.from({ length: 12 }, (_, index) => ({
    id: `choice-${index}`,
    label: index === 10 ? "詳細 🧭" : index === 11 ? "Unsafe\u001b]8;;label\u0007" : `Choice ${index}`,
...
test("settings search reports no matches and all presentation remains width-safe", () => {
  ...
  assert.ok(plainRender(harness.component, 40).some((line) => line.includes("No matching settings")));
  harness.component.handleInput("\u007f");
  assert.ok(plainRender(harness.component, 40).some((line) => line.includes("介面顯示設定")));

  const empty = componentHarness({ kind: "settings", title: "Empty settings", items: [] }, { plainTheme: true });
  assert.ok(plainRender(empty.component, 40).some((line) => line.includes("No settings available")));
...
test("settings sanitize terminal controls from bracketed search paste", () => {
  const harness = componentHarness(settingsScreen, { plainTheme: true });
  const pasted = "\u001b[200~man\u001b]8;;https://unsafe.example\u0007ual\u009dtitle\u009c\u001b[201~";
...
test("searchable multi-select filters label and declared search text by stable raw id", async () => {
  ...
  await harness.component.waitForPending();
  assert.deepEqual(requests, [{ itemId: "read\u0007raw", selected: true, previousSelected: false }]);
...
test("searchable multi-select keeps actions available for no matches and distinguishes empty", async () => {
  ...
  const empty = componentHarness({ ...screen, items: [] }, { plainTheme: true, keybindings: inputFriendlyKeybindings });
