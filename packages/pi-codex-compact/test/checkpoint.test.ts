...
test("projects the persisted summary without regenerating version-dependent prose", () => {
  ...
  for (const persistedSummary of [
    ...
    assert.equal(project([summary, user("changed", 2), after], details, persistedSummary), undefined);
  }
...
test("projects resumed repeated compaction when Pi interleaves an older summary", () => {
  ...
  const secondDetails = checkpoint([firstKept, secondKept, thirdKept, fourthKept], "checkpoint-second");
  const entries = [
    messageEntry("first-kept", null, firstKept),
    messageEntry("second-kept", "first-kept", secondKept),
    compactionEntry("first-compaction", "second-kept", "first-kept", firstDetails, "2026-01-01T00:00:03.000Z"),
    messageEntry("third-kept", "first-compaction", thirdKept),
    messageEntry("fourth-kept", "third-kept", fourthKept),
    compactionEntry("second-compaction", "fourth-kept", "first-kept", secondDetails, "2026-01-01T00:00:06.000Z"),
