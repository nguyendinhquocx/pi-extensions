...
import { renderCompletionSummary, renderProgressWidget, sanitizeProgressText } from "../src/progress-renderer.js";
import { identityTheme } from "./progress-harness.js";
...
for (const [name, raw, expected] of [
  ...
  test(`Progress shared sanitizer: ${name}`, () => {
    assert.equal(sanitizeProgressText(raw), expected);
    const steps = [{ text: raw, status: "pending" as const }];
    const before = structuredClone(steps);
    renderProgressWidget(steps, identityTheme().theme, 80);
    assert.deepEqual(steps, before);
  });
}

for (const width of [0, 1, 2, 10, 80]) {
  test(`Progress separators preserve row ownership at width ${width}`, () => {
    const { theme, calls } = identityTheme();
    for (const lines of [
      renderCompletionSummary(2, theme, width),
      renderProgressWidget([{ text: "pending", status: "pending" }], theme, width),
    ]) {
