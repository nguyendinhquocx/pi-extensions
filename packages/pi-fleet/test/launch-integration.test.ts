...
posixTest("fake tmux launches a real child endpoint, propagates cwd and launch id, and delivers kickoff", async () => {
  ...
  try {
    ...
    await waitForChildEvent(
      childEvents,
      (event) => event.type === "message" && (event.message as { text?: string }).text === "Run integration task",
    );
